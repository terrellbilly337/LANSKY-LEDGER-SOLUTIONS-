...
import { Package, TrendingUp, Wallet, ArrowRight, AlertCircle, ChevronDown, ChevronUp } from 'lucide-react';
...
const AccountCard = ({ 
    ...
    return (
        <div 
            onClick={onClick}
            className="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-sm border border-slate-100 dark:border-slate-700/50 cursor-pointer transition-colors hover:border-[var(--primary)] dark:hover:border-[var(--primary)] group h-full"
        >
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 landscape:grid-cols-3 lg:grid-cols-3 gap-6 pt-4">
        {/* Main Chart */}
        <div className="landscape:col-span-2 lg:col-span-2 bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-sm border border-slate-100 dark:border-slate-700/50 flex flex-col h-full">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-sm border border-slate-100 dark:border-slate-700/50 flex flex-col h-full">
          <div className="flex items-center justify-between mb-4">
             <div className="flex items-center gap-2">
                ...
                <button 
                    ...
                    className="p-1 hover:bg-slate-100 dark:hover:bg-slate-700 rounded transition-colors text-slate-400"
                >
                    {isRecentOpen ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                </button>
             </div>
             <button 
                onClick={() => navigate('/inventory')}
          ...
          {isRecentOpen && (
              <div className="flex-grow overflow-y-auto pr-1 -mx-2 px-2 animate-fade-in">
